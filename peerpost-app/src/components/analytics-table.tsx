...
export function AnalyticsTable({
	...
	const [sortKey, setSortKey] = useState<
		keyof AnalyticsMetrics | "publishedAt"
	>("publishedAt");
	const [expanded, setExpanded] = useState<string | null>(null);

	const view = useMemo(() => {
		let r =
			filter === "all" ? rows : rows.filter((x) => x.profileId === filter);
		r = [...r].sort((a, b) => {
		...
		<div className="space-y-3">
			...
			{view.length === 0 ? (
				<p className="rounded-lg border border-black/10 p-4 text-sm opacity-60">
					No analytics yet. Publish a post, then Refresh (metrics may take a
					while to populate on each platform).
				</p>
			) : (
							...
							{view.map((r) => (
								<Fragment key={r.postpeerPostId}>
									<tr
										...
										onClick={() =>
											setExpanded(
												expanded === r.postpeerPostId ? null : r.postpeerPostId,
											)
										}
									>
											...
											<td colSpan={COLS.length + 1} className="px-3 py-2">
												<div className="space-y-1">
													{(r.platforms ?? []).map((p) => (
														...
														>
															<span className="w-20 font-medium uppercase">
																{p.platform}
															</span>
															{COLS.map((c) => (
